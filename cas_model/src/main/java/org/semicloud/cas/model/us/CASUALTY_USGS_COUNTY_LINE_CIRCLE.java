...
public class CASUALTY_USGS_COUNTY_LINE_CIRCLE extends BaseModel {
     ...
     */
    public CASUALTY_USGS_COUNTY_LINE_CIRCLE(ModelInitilizer initilizer, String modelName) {
    ...
    public String getJson() {
        ...
        if (getCountryAttribute().getCountryAbbr().equals("CN")) {
            ...
            List<IntensityLineCircle> intensityCircles = getIntensityLineCircles()
                    .stream().filter(c -> c.getIntensity() >= 8.0).collect(Collectors.toList());
            // 显示一下过滤后的烈度值，即8度以上的
