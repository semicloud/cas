 ...
 * 3小时研判任务
 */
public class Hour3Task extends BaseTask implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        log.info(text("{0},mag:{1} hour3 analysis task startup @ {2}", event.getInfo(), event.getMagnitude(),
                new Date()));
        for (BaseModel m : invoker.getModels(TimePoint.HOUR_3, event.getEqID())) {
            if (m != null) {
                if (m.process()) {
                    log.info(text("model {0} at {1} processed -> {2}", m.getName(), TimePoint.HOUR_3, true));
                }
            }
        }
    }
